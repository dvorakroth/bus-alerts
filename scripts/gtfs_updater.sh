...
pushd ${DIR}
    ...
    psql -n $POSTGRES_DSN -c "drop table if exists stops_trip_count; drop table if exists actual_lines; drop view if exists stoptimes_int; drop table if exists agency, cities, routes, shapes, stops, stoptimes, translations, trip_id_to_date, trips, calendar, mot_clusters cascade;"
    ...
    echo -n "creating stops_trip_count table:    "
    psql $POSTGRES_DSN -f stops_trip_count.sql
    echo -n "grouping routes into actual lines:    "
